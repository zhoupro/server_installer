...
if [ ! -f php-5.5.38.tar.gz ];then
    wget http://mirrors.sohu.com/php/php-5.5.38.tar.gz  -O  php-5.5.38.tar.gz
fi
...
make install
cd ..
cp ./php-5.5.38/php.ini-production $install_dir/server/php/etc/php.ini
#adjust php.ini
...
install -v -m755 ./php-5.5.38/sapi/fpm/init.d.php-fpm  /etc/init.d/php-fpm
